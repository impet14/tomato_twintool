...
j0_init = rosparam.get_param("/joint0_controller/motor/init")
j1_init = rosparam.get_param("/joint1_controller/motor_master/init")
j2_init = rosparam.get_param("/joint2_controller/motor/init")
j3_init = rosparam.get_param("/joint3_controller/motor/init")
j4_init = rosparam.get_param("/joint4_controller/motor/init")
j5_init = rosparam.get_param("/joint5_controller/motor/init")
j6_init = rosparam.get_param("/joint6_controller/motor/init")
...
def process(msg):
	...
	for x in msg.motor_states:
		...
		if(x.id != 12):
			...
			elif(x.id in MX_list):
				#in case x.id == 11,14
				resolution = 4095
				deg_range  = 360.0
				if(x.id >= 12):
					joint_states.name.append(joints[x.id-start_ID-1])
					joint_states.position.append((x.position-offset[x.id-start_ID-1])*(deg_range/resolution)*(pi/180))
				else:
					joint_states.name.append(joints[x.id-start_ID])
